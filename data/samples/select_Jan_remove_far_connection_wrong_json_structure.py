...
for r in df.itertuples():
    ...
    samples.append({
        ...
        "route": route,
        "start_time": to_iso(r.local_datetime_start),
...
# =========================
# GROUP + BUILD LINKED TRIPS (OLD LOGIC)
# =========================
...
for lid, trips in groups.items():
    ...
    origin = {**trips[0]["origin"], "start_time": trips[0]["start_time"]}
    destination = {**trips[-1]["destination"], "end_time": trips[-1]["start_time"]}
...
for ORIG, DEST in OD_PAIRS:
    subset = [
        ...
        if lt["origin"]["geohash"] and lt["destination"]["geohash"]
        and df.loc[df["linked_trip_id"] == lt["linked_trip_id"], "GEOID_orig"].iloc[0] == ORIG
